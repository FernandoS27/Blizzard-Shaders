...
A worked example of :mod:`shader_diff` for the Warcraft-III Reforged foliage
pixel shader. ``foliage_ps`` reuses the SAME HD-family constant-buffer light /
IBL / shadow layout as ``hd_ps`` (see ``tools/shader_diff_hd_ps.py``), so this
driver drives the identical discriminants:

  cb1[0].x   shadow-cascade count (ult / frustum-loop bound; SHAD perms)
  cb2[20].z  light count           (ult loop bound + directional gate)
  cb2[20].w  useNdf  -> FORCED 0.0 (specular-AA depends on real screen-space
             derivatives, which a single-invocation interpreter can't match)
  cb2[19].xy IBL probe extents     (probe bound iff x*y != 0)
  cb2[19].z  IBL blend factor
  cb2[4i+21] per-light ambient / [+22] diffuse / [+23] position(.w==0 == directional)
  cb3[0].y   debug mode (int 0..8; DBG perms 64..127); cb3[0].x debug flags

Per-semantic varyings (retail one-semantic-per-register, slang matches raw idx):
  COLOR0=vertColor, TEXCOORD0=uv, TEXCOORD1=viewPos(view-space, .xyz),
  TEXCOORD2=normalWS(unit), TEXCOORD3=tangentWS(unit,.w=+/-1 handedness),
  TEXCOORD4/5/6=shadow-cascade clip coords (SHAD perms only).

foliage_ps is an MRT shader: MRT perms write o0/o1/o2, the rest only o0; a few
perms (e.g. 002/003) have no output at all (empty prepass shader). The driver
reads each retail perm's output signature and compares exactly those registers.
SV_ISFRONTFACE is declared in every perm but never actually read, so the
front-face sysval is irrelevant here (still provided for safety).

Run from the repo root::

    python tools/shader_diff_foliage_ps.py                 # full 128-perm sweep
    python tools/shader_diff_foliage_ps.py --perms 0,1,4   # specific perms
    python tools/shader_diff_foliage_ps.py --trials 60
"""
...
def foliage_inputs(seed):
    ...
    return {
        ...
        ("TEXCOORD", 0): [f2b(r.uniform(-2, 2)) for _ in range(4)],           # uv (+extra)
        ("TEXCOORD", 1): [f2b(r.uniform(-20, 20)) for _ in range(3)]
                         + [f2b(r.uniform(-5, 5))],                           # viewPos
...
def foliage_cbufs(seed):
    ...
    cb1[0][0] = i2b(cascades)                # shadow cascade count
    cb3[0][0] = i2b(rng.getrandbits(3))      # debug flags (AND-tested bits 1,2)
    cb3[0][1] = i2b(dbg)                      # debug mode (ieq 1..8, if_z 0)
    cb2[20][2] = i2b(lights)                  # light count / loop bound
    cb2[20][3] = f2b(0.0)                     # useNdf OFF (specular-AA unmatchable)
    cb2[16][0] = f2b(rng.uniform(0, 1))
    cb2[16][1] = f2b(rng.uniform(0, 1))
    cb2[19][2] = f2b(rng.uniform(0, 1))       # IBL blend
    # IBL probe bound iff extent.x*extent.y != 0
...
def foliage_sysvals(seed):
    return {'is_front_face': 0xFFFFFFFF if (seed & 1) else 0}


# --- perm feature label (best-effort, from cb usage groupings) ------------
# foliage_ps 128 perms. Observed structure: bit 64 => DBG (cb3), and within
# each 32-perm block the SHAD variants (cb1) recur. A precise decode isn't
# needed for the report, so label by the coarse feature bits we confirmed.

def feat(idx):
    f = []
    if idx & 1:   f.append("MRT")       # perm_001 writes o0/o1/o2
    if idx & 64:  f.append("DBG")       # perms 64-127 read cb3 debug
    return "+".join(f) if f else "base"


def output_regs_for(prog):
    """SV_Target registers this shader writes (empty for the prepass perms)."""
...
def main(argv=None):
    ...
    for idx in perms:
        ...
        if not oregs:                    # empty prepass shader -> nothing to diff
            print(f"  skip    perm_{idx:03d} {feat(idx)}: no SV_Target output")
            continue
        res = compare(prog_s, prog_r, trials=args.trials, output_regs=oregs,
                      tol=args.tol, inputs_fn=foliage_inputs, cbufs_fn=foliage_cbufs,
                      sysvals_fn=foliage_sysvals)
    ...
    print(f"\n=== foliage_ps: {len(perms)} perms x {args.trials} trials ===")
